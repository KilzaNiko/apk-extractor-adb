...
echo.
echo  %C_CYAN%╔══════════════════════════════════════════════════════╗%C_RESET%
echo  %C_CYAN%║%C_BOLD%%C_WHITE%            APK EXTRACTOR  %C_CYAN%^|%C_WHITE%  ADB Tool              %C_CYAN%║%C_RESET%
echo  %C_CYAN%╚══════════════════════════════════════════════════════╝%C_RESET%
echo.
...
if %errorlevel%==0 (
    ...
    echo  %C_GREEN%[OK]%C_RESET% ADB encontrado en el sistema: !ADB_EXE!
...
if exist "C:\platform-tools\adb.exe" (
    ...
    echo  %C_GREEN%[OK]%C_RESET% ADB encontrado en C:\platform-tools
    echo !ADB_EXE!>"%CONFIG_FILE%"
    timeout /t 2 /nobreak >nul
    goto MAIN_MENU
)
...
echo.
echo  %C_CYAN%╔══════════════════════════════════════════════════════╗%C_RESET%
echo  %C_CYAN%║%C_BOLD%%C_WHITE%            APK EXTRACTOR  %C_CYAN%^|%C_WHITE%  Configuracion         %C_CYAN%║%C_RESET%
echo  %C_CYAN%╚══════════════════════════════════════════════════════╝%C_RESET%
echo.
...
echo.
echo  %C_YELLOW%[1]%C_RESET% Ingresar la ruta donde tengo ADB instalado
echo  %C_YELLOW%[2]%C_RESET% Descargar ADB ahora (se instalara en C:\platform-tools)
echo  %C_YELLOW%[3]%C_RESET% Volver al menu principal
echo.
...
if "%SETUP_CHOICE%"=="3" (
    if defined ADB_EXE (
        if exist "!ADB_EXE!" goto MAIN_MENU
    )
    echo.
    echo  %C_RED%[!]%C_RESET% No se puede volver sin ADB configurado.
    pause
)
goto SETUP_ADB
...
echo %ADB_EXE%>"%CONFIG_FILE%"
echo.
echo  %C_GREEN%[OK]%C_RESET% Ruta guardada correctamente.
timeout /t 2 /nobreak >nul
goto MAIN_MENU
...
echo.
echo  %C_CYAN%╔══════════════════════════════════════════════════════╗%C_RESET%
echo  %C_CYAN%║%C_BOLD%%C_WHITE%            APK EXTRACTOR  %C_CYAN%^|%C_WHITE%  Descargando ADB      %C_CYAN%║%C_RESET%
echo  %C_CYAN%╚══════════════════════════════════════════════════════╝%C_RESET%
echo.
...
echo %ADB_EXE%>"%CONFIG_FILE%"
echo.
echo  %C_GREEN%[OK]%C_RESET% ADB instalado correctamente en C:\platform-tools
echo  %C_GREEN%[OK]%C_RESET% Ruta guardada en configuracion.
...
echo.
echo  %C_CYAN%╔══════════════════════════════════════════════════════╗%C_RESET%
echo  %C_CYAN%║%C_BOLD%%C_WHITE%            APK EXTRACTOR  %C_CYAN%^|%C_WHITE%  Menu Principal        %C_CYAN%║%C_RESET%
echo  %C_CYAN%╚══════════════════════════════════════════════════════╝%C_RESET%
echo.
echo  %C_GRAY%ADB: %ADB_EXE%%C_RESET%
echo.
echo  %C_YELLOW%[1]%C_RESET% Listar dispositivos conectados
echo  %C_YELLOW%[2]%C_RESET% Depuracion inalambrica (WiFi)
echo  %C_YELLOW%[3]%C_RESET% Dispositivos guardados
echo  %C_YELLOW%[4]%C_RESET% Cambiar ruta de ADB
echo  %C_RED%[5]%C_RESET% Salir
echo.
...
echo.
echo  %C_CYAN%╔══════════════════════════════════════════════════════╗%C_RESET%
echo  %C_CYAN%║%C_BOLD%%C_WHITE%            APK EXTRACTOR  %C_CYAN%^|%C_WHITE%  Dispositivos          %C_CYAN%║%C_RESET%
echo  %C_CYAN%╚══════════════════════════════════════════════════════╝%C_RESET%
echo.
...
:: Filtrar solo lineas con devices autorizados a un archivo limpio
set "TMP_DEVCLEAN=%TEMP%\apk_devices_clean.tmp"
if exist "%TMP_DEVCLEAN%" del "%TMP_DEVCLEAN%"
findstr /c:"	device" "%TMP_DEVLIST%" >"%TMP_DEVCLEAN%" 2>nul

:: Leer dispositivos del archivo filtrado
for /f "usebackq tokens=1 delims=	" %%s in ("%TMP_DEVCLEAN%") do (
    set /a DEVICE_COUNT+=1
    set "DEVICE_!DEVICE_COUNT!=%%s"
    set "DEVNAME_!DEVICE_COUNT!="
)
del "%TMP_DEVLIST%" >nul 2>&1
del "%TMP_DEVCLEAN%" >nul 2>&1

:: Buscar nombres personalizados para cada dispositivo
if exist "!NAMES_FILE!" (
    for /l %%i in (1,1,!DEVICE_COUNT!) do (
        set "CUR_SERIAL=!DEVICE_%%i!"
        for /f "usebackq delims=" %%n in (`findstr /c:"\"!CUR_SERIAL!\"" "!NAMES_FILE!" 2^>nul`) do (
            set "NMATCH=%%n"
            for /f "tokens=2 delims=:" %%v in ("!NMATCH!") do (
                ...
                set "NVAL=!NVAL:"=!"
                set "NVAL=!NVAL:,=!"
                if not "!NVAL!"=="" set "DEVNAME_%%i=!NVAL!"
            )
        )
    )
)
...
echo.
for /l %%i in (1,1,%DEVICE_COUNT%) do (
    set "SHOW_DEV=!DEVICE_%%i!"
    if defined DEVNAME_%%i (
        echo  [%%i] !DEVNAME_%%i!  ^(!SHOW_DEV!^)
    ) else (
        echo  [%%i] !SHOW_DEV!
    )
)
...
echo.
echo  %C_CYAN%╔══════════════════════════════════════════════════════╗%C_RESET%
echo  %C_CYAN%║%C_BOLD%%C_WHITE%            APK EXTRACTOR  %C_CYAN%^|%C_WHITE%  Dispositivo           %C_CYAN%║%C_RESET%
echo  %C_CYAN%╚══════════════════════════════════════════════════════╝%C_RESET%
echo.
...
echo  └─────────────────────────────────────────────────────┘
echo.
echo  %C_YELLOW%[1]%C_RESET% Listar aplicaciones instaladas
echo  %C_YELLOW%[2]%C_RESET% Asignar nombre al dispositivo
echo  %C_YELLOW%[3]%C_RESET% Volver a la lista de dispositivos
echo  %C_YELLOW%[4]%C_RESET% Volver al menu principal
echo.
...
if defined NEW_DEV_NAME (
    if not "%NEW_DEV_NAME%"=="" (
        echo.
        echo  %C_GREEN%[OK]%C_RESET% Dispositivo renombrado a: %NEW_DEV_NAME%
    ) else (
        echo.
        echo  %C_GREEN%[OK]%C_RESET% Nombre eliminado.
    )
) else (
    echo.
    echo  %C_GREEN%[OK]%C_RESET% Nombre eliminado.
...
echo.
echo  %C_CYAN%╔══════════════════════════════════════════════════════╗%C_RESET%
echo  %C_CYAN%║%C_BOLD%%C_WHITE%            APK EXTRACTOR  %C_CYAN%^|%C_WHITE%  Aplicaciones          %C_CYAN%║%C_RESET%
echo  %C_CYAN%╚══════════════════════════════════════════════════════╝%C_RESET%
echo.
...
echo.
echo  %C_YELLOW%[1]%C_RESET% Solo aplicaciones de terceros (instaladas por el usuario)
echo  %C_YELLOW%[2]%C_RESET% Todas las aplicaciones (incluye sistema)
echo  %C_YELLOW%[0]%C_RESET% Volver
echo.
...
for /f "tokens=2 delims=:" %%p in ('type "%TMP_PKG%"') do (
    ...
    set "APKC=0"
    set "APKSZ=0"
    set "TMP_PATHS_I=%TEMP%\apk_paths_i.tmp"
    if exist "!TMP_PATHS_I!" del "!TMP_PATHS_I!"

    :: Guardar rutas del paquete en archivo temporal
    "%ADB_EXE%" -s "%CURRENT_DEVICE%" shell pm path !CPKG! 2>nul > "!TMP_PATHS_I!"

    :: Contar cuantos APKs tiene
    for /f %%x in ('type "!TMP_PATHS_I!" ^| find /c "package:"') do set /a APKC=%%x

    :: Obtener tamaño de cada APK con du -b (via archivo temporal para evitar escaping)
    set "TMP_DU=%TEMP%\apk_du.tmp"
    for /f "tokens=2 delims=:" %%r in ('type "!TMP_PATHS_I!"') do (
        set "APATH=%%r"
        :: Limpiar espacios y CR del path
        set "APATH=!APATH: =!"
        if not "!APATH!"=="" (
            if exist "!TMP_DU!" del "!TMP_DU!"
            "%ADB_EXE%" -s "%CURRENT_DEVICE%" shell du -b "!APATH!" 1>"!TMP_DU!" 2>nul
            for /f "tokens=1" %%z in ('type "!TMP_DU!" 2^>nul ^| findstr /r "^[0-9]"') do (
                set "RAWSZ=%%z"
                if not "!RAWSZ!"=="" (
                    set /a APKSZ+=!RAWSZ!/1024 2>nul
                )
            )
        )
    )
...
echo.
echo  %C_CYAN%╔══════════════════════════════════════════════════════╗%C_RESET%
echo  %C_CYAN%║%C_BOLD%%C_WHITE%            APK EXTRACTOR  %C_CYAN%^|%C_WHITE%  Detalle de App        %C_CYAN%║%C_RESET%
echo  %C_CYAN%╚══════════════════════════════════════════════════════╝%C_RESET%
echo.
echo  Dispositivo : %DEV_BRAND% %DEV_MODEL%
...
if exist "%APK_OUT_FILE%" (
    ...
    echo  %C_GREEN%║  [OK] APK extraido exitosamente!                    ║%C_RESET%
    echo  %C_GREEN%╚══════════════════════════════════════════════════════╝%C_RESET%
    echo.
    ...
    echo.
    echo  %C_RED%╔══════════════════════════════════════════════════════╗%C_RESET%
    echo  %C_RED%║  [!] ERROR al extraer el APK                        ║%C_RESET%
    echo  %C_RED%║  Verifica que tengas permisos en la carpeta         ║%C_RESET%
    echo  %C_RED%║  destino y que el APK no este protegido.            ║%C_RESET%
    echo  %C_RED%╚══════════════════════════════════════════════════════╝%C_RESET%
    echo.
)
...
echo.
echo  %C_CYAN%╔══════════════════════════════════════════════════════╗%C_RESET%
echo  %C_CYAN%║%C_BOLD%%C_WHITE%            APK EXTRACTOR  %C_CYAN%^|%C_WHITE%  Compilar XAPK         %C_CYAN%║%C_RESET%
echo  %C_CYAN%╚══════════════════════════════════════════════════════╝%C_RESET%
echo.
echo  Paquete: %SELECTED_PKG%
...
if exist "%XAPK_OUT%" (
    echo.
    echo  %C_GREEN%╔══════════════════════════════════════════════════════╗%C_RESET%
    echo  %C_GREEN%║  [OK] XAPK compilado exitosamente!                  ║%C_RESET%
    echo  %C_GREEN%╚══════════════════════════════════════════════════════╝%C_RESET%
    echo.
    ...
    echo.
    echo  %C_RED%╔══════════════════════════════════════════════════════╗%C_RESET%
    echo  %C_RED%║  [!] ERROR al crear el XAPK                         ║%C_RESET%
    echo  %C_RED%║  Verifica permisos en la carpeta de destino         ║%C_RESET%
    echo  %C_RED%╚══════════════════════════════════════════════════════╝%C_RESET%
    echo.
)
...
echo.
echo  %C_CYAN%╔══════════════════════════════════════════════════════╗%C_RESET%
echo  %C_CYAN%║%C_BOLD%%C_WHITE%            APK EXTRACTOR  %C_CYAN%^|%C_WHITE%  WiFi Debug            %C_CYAN%║%C_RESET%
echo  %C_CYAN%╚══════════════════════════════════════════════════════╝%C_RESET%
echo.
...
echo.
echo  %C_YELLOW%[1]%C_RESET% Iniciar vinculacion
echo  %C_YELLOW%[0]%C_RESET% Volver al menu principal
echo.
set /p WIFI_CHOICE="  Elige una opcion [0-1]: "
if "%WIFI_CHOICE%"=="0" goto MAIN_MENU
if not "%WIFI_CHOICE%"=="1" goto WIRELESS_MENU
echo.
echo  ┌─────────────────────────────────────────────────────┐
echo  │  %C_BOLD%Paso 1: Emparejamiento%C_RESET%                             │
echo  └─────────────────────────────────────────────────────┘
echo.
...
echo.
echo  %C_GREEN%[OK]%C_RESET% Emparejamiento exitoso.
echo.
echo  ┌─────────────────────────────────────────────────────┐
...
echo.
echo  %C_GREEN%[OK]%C_RESET% Conectado exitosamente a %WIFI_IP%:%WIFI_CONN_PORT%
echo.
...
echo  %C_GREEN%[OK]%C_RESET% Dispositivo guardado para reconexion futura.
echo.
pause
goto MAIN_MENU
...
echo.
echo  %C_CYAN%╔══════════════════════════════════════════════════════╗%C_RESET%
echo  %C_CYAN%║%C_BOLD%%C_WHITE%            APK EXTRACTOR  %C_CYAN%^|%C_WHITE%  Dispositivos Guardados%C_CYAN%║%C_RESET%
echo  %C_CYAN%╚══════════════════════════════════════════════════════╝%C_RESET%
echo.
...
echo.
echo  %C_YELLOW%[C]%C_RESET% Conectar a un dispositivo
echo  %C_YELLOW%[D]%C_RESET% Eliminar un dispositivo
echo  %C_YELLOW%[0]%C_RESET% Volver al menu principal
echo.
...
if %errorlevel%==0 (
    echo  %C_GREEN%[OK]%C_RESET% Conectado a !SC_LABEL!
    echo.
    pause
    goto LIST_DEVICES
) else (
...
echo  %C_GREEN%[OK]%C_RESET% Dispositivo eliminado.
timeout /t 2 /nobreak >nul
goto SAVED_DEVICES_MENU
